...
# 🆕 Uproszczona struktura: Level 1 → Level 2 → Level 3 (konkret)
...
print("="*70)
print("🔍 FAZA 4: EKSTRAKCJA ZABIEGÓW")
print("="*70)

def extract_treatments_from_text(domain: str, text: str) -> List[str]:
    text_chunk = text[:8000]
    
    system = "Jesteś ekspertem medycyny estetycznej. Wyciągasz nazwy zabiegów z tekstu."
    user = f"""
...
Jedna nazwa w linii, bez numeracji.
"""
...
def normalize_batch(treatments: List[str]) -> Dict[str, str]:
    ...
    system = "Jesteś ekspertem medycyny estetycznej. Normalizujesz nazwy zabiegów."
    user = f"""
...
Wybierz JEDNĄ najbardziej ogólną nazwę (bez numeru).
"""
...
Tylko opis, bez nagłówków.
"""
        
        try:
            return clean_text(or_chat([{"role":"system","content":system},{"role":"user","content":user}], temperature=0.3, max_tokens=200))
        except:
            return ""
