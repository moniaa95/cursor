...
# 🆕 ZOPTYMALIZOWANA (300-600 najpopularniejszych zabiegów)
...
PROJECT_NAME = "baza_zabiegow_optimized"  # @param {type:"string"}
...
MAX_TREATMENTS_PER_LEVEL2 = 12  # @param {type:"integer"}

# @title 🧹 Filtry
MIN_TEXT_LEN = 5     # @param {type:"integer"}
MAX_TEXT_LEN = 120   # @param {type:"integer"}
SEMANTIC_SIM_THRESHOLD = 0.88  # @param {type:"slider", min:0.75, max:0.95, step:0.01}
...
def deduplicate_semantic(treatments: List[str], threshold: float = 0.88) -> List[str]:
    """Usuwa semantyczne duplikaty (bardzo podobne zabiegi)."""
    if len(treatments) <= 1:
        return treatments
    
    embeddings = embed_texts(treatments)
...
print("="*70)
print("🏗️ FAZA 3: BUDOWA STRUKTURY (LLM)")
print("="*70)
...
Dla KAŻDEGO Level 1 wypisz najważniejsze podkategorie Level 2.
...
Wypisz WSZYSTKIE istotne Level 2 dla każdego Level 1!
"""
...
# ============================================================
# 🆕 FAZA 4: TOP NAJPOPULARNIEJSZYCH ZABIEGÓW (OPTIMIZED!)
# ============================================================

print("="*70)
print(f"💉 FAZA 4: TOP {MAX_TREATMENTS_PER_LEVEL2} NAJPOPULARNIEJSZYCH ZABIEGÓW")
print("="*70)

def generate_top_treatments(level1: str, level2: str, max_count: int = 12) -> List[str]:
    system = f"""
Jesteś ekspertem medycyny estetycznej. Wypisujesz TOP {max_count} NAJPOPULARNIEJSZYCH zabiegów w Polsce dla danej podkategorii.

WAŻNE:
- Tylko {max_count} najpopularniejszych, oferowanych przez większość klinik
- Nazwy OGÓLNE (bez marek: Dermapen→mezoterapia mikroigłowa, Botox→toksyna botulinowa)
- Konkretne, nie ogólne (✅"Manicure hybrydowy" ❌"Pielęgnacja paznokci")
- Bez rzadkich/niszowych zabiegów
"""
    
    user = f"""
Kategoria: {level1} → {level2}

Wypisz TOP {max_count} NAJPOPULARNIEJSZYCH zabiegów w Polsce dla tej kategorii.

Kryteria:
✅ Oferowane przez większość klinik
✅ Często wyszukiwane przez pacjentów
✅ Popularne w Polsce w 2025

Przykłady:
✅ Manicure hybrydowy (bardzo popularne)
✅ Depilacja laserowa bikini (popularne)
✅ Toksyna botulinowa zmarszczek międzybrwiowych (popularne)
❌ Mezoterapia kwasem traneksamowym lewego kolana (zbyt niszowe)

Wypisz TYLKO {max_count} najpopularniejszych (jedna nazwa w linii, bez numeracji).
"""
...
for idx, row in tqdm(df_structure.iterrows(), total=len(df_structure), desc="Generowanie TOP zabiegów"):
    treatments = generate_top_treatments(row["level1"], row["level2"], max_count=MAX_TREATMENTS_PER_LEVEL2)
...
print("="*70)
print("🕷️ FAZA 5: SCRAPOWANIE DOMEN")
print("="*70)
...
# ============================================================
# FAZA 8: DEDUPLIKACJA SEMANTYCZNA
# ============================================================

print("="*70)
print("🧹 FAZA 8: DEDUPLIKACJA SEMANTYCZNA")
print("="*70)
...
if len(df_mapped_scraped) > 0:
    ...
    df_combined = df_treatments[["level1", "level2", "level3", "source", "domain"]].copy()

# Deduplikacja po tekstowej nazwie
df_combined = df_combined.sort_values("source", ascending=False)  # scraped > llm
df_combined = df_combined.drop_duplicates(subset=["level1", "level2", "level3"], keep="first").reset_index(drop=True)

print(f"📌 Po deduplikacji tekstowej: {len(df_combined)} zabiegów")

# Deduplikacja semantyczna per kategoria Level 2
print("🔹 Deduplikacja semantyczna (usuwam bardzo podobne)...")

final_treatments = []

for (l1, l2), group in tqdm(df_combined.groupby(["level1", "level2"]), desc="Deduplikacja"):
    treatments_list = group["level3"].tolist()
    
    if len(treatments_list) > 1:
        unique_treatments = deduplicate_semantic(treatments_list, threshold=SEMANTIC_SIM_THRESHOLD)
    else:
        unique_treatments = treatments_list
    
    for t in unique_treatments:
        row = group[group["level3"] == t].iloc[0]
...
df_final = pd.DataFrame(final_treatments)
...
# Podsumowanie
print("\n" + "="*70)
print("🎉 GOTOWE! ZOPTYMALIZOWANA TAKSONOMIA - PODSUMOWANIE:")
print("="*70)
...
print("\n" + "="*70)
print("✅ TAKSONOMIA 300-600 ZABIEGÓW GOTOWA!")
print("✅ Depilacja w Kosmetologii ✓")
print("✅ Tylko najpopularniejsze zabiegi ✓")
